view: vas_order_graph {
  derived_table: {
    sql:    with upr as (select extract(month from date) mt , upr.city, upr.micromarket, upr.residence, max(upr.moved_in_residents) as moved_in_residents, count(distinct upr.id) as consumed_meals,
...
vo as (select extract(month from date) mt, city, micromarket,residence, count(distinct vo.id) as total_orders, sum(case when rating is not null then 1 else 0 end) as rated_orders,
