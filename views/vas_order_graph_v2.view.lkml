view: vas_order_graph_v2 {
  derived_table: {
    sql:    with upr as (select extract(year from upr.date) yr,extract(month from upr.date) mt,upr.city, upr.micromarket,residence, max(moved_in_residents) as moved_in_residents, count(distinct upr.id) as consumed_meals,
