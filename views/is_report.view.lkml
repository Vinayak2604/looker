view: is_report {
  ...
  measure: total_calls_yesterday {
    type: count_distinct
    sql:  case when date(${lead_created_time}) = date_add(current_date,INTERVAL -1 DAY) and ${call_status} is not null then ${id} end  ;;
  }

  measure: total_calls_L7D {
    type: count_distinct
    sql:  case when date(${lead_created_time}) >= date_add(current_date,INTERVAL -7 DAY) and date(${lead_created_time}) <= date_add(current_date,INTERVAL -1 DAY) and ${call_status} is not null then ${id} end  ;;
  ...
  measure: total_calls_L30D {
    type: count_distinct
    sql:  case when date(${lead_created_time}) >= date_add(current_date,INTERVAL -30 DAY) and date(${lead_created_time}) <= date_add(current_date,INTERVAL -1 DAY) and ${call_status} is not null then ${id} end  ;;
   link: {
