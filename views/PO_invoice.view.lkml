...
derived_table: {
  ...
  case
        when pd.po_type = 'RENTAL'
    or pd.po_type = 'NON_RENTAL'
    or pd.po_type = 'SERVICE_PO' then DP.location_name
  end as property,
  ...
  case
    when po_status not in ('GSRI_COMPLETED', 'APPROVED', 'SHORTCLOSED') then datediff(day,
    po_start_date,
    po_completion_date)
    when po_status in ('GSRI_COMPLETED', 'APPROVED', 'SHORTCLOSED')
    and ibd.created_at is null then datediff(day,
    po_completion_date,
    current_date)
    when (ibd.created_at is not null
    and L1_approval_at is null) then datediff(day,
    ibd.created_at ,
    current_date)
    when L1_approval_at is not null
    and L1_reject_at is not null then datediff(day,
    ibd.created_at,
    L1_reject_at)
    when L1_approval_at is not null
    and L1_reject_at is null
    and L2_approval_at is null then datediff(day,
    L1_approval_at,
    current_date)
    when L2_approval_at is not null
    and L2_reject_at is not null then datediff(day,
    ibd.created_at,
    L2_reject_at)
    when L2_approval_at is not null
    and L2_reject_at is null then 0
  end ageing,
...
left join
  ...
  from
    ...
    where
      iaa.__hevo__marked_deleted is false
      and iaa.id in (
      select
        id
      from
        ...
        select
          ...
          max(id) id
        from
          stanza.erp_erp_invoice_invoice_approvals
        group by
          1))) as cc
  left join
    (
...
left join(
  select
    kk.invoice_uuid,
    coalesce(sum(kk.item_base_amount - kk.item_gst), 0) as base_amount,
      coalesce(sum(dd.other_fee_base_amount-dd.other_fee_gst), 0) as Other_charges
  from
    (
    select
      invoice_uuid,
