view: vas_ageing_aov_bucket_cm {
  derived_table: {
    sql: with a as (select id,yr,mt, date,  residence,  micromarket,  city, cafe, mealtime, order_code, user_type,  order_type,
          ...
          select mt, case when final_total_amount <=25 then 'a. 0-25' when final_total_amount <=50 then 'b. 25-50'
  ...
  measure: 0_25 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_aov_bucket_cm.0_25_html._rendered_value}} </p>;;
  }
  ...
  measure: 25_50 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_aov_bucket_cm.25_50_html._rendered_value}} </p> ;;
  ...
  measure: 50_75 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_aov_bucket_cm.50_75_html._rendered_value}}</p> ;;
  ...
  measure: 75_100 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_aov_bucket_cm.75_100_html._rendered_value}} </p>;;
  ...
  measure: _100 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_aov_bucket_cm._100_html._rendered_value}} </p>;;
