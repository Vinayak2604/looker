view: vas_ageing_order_bucket_cm {
  derived_table: {
    sql: with a as (select id,yr,mt, date,  residence,  micromarket,  city, cafe, mealtime, order_code, user_type,  order_type,
          ...
          select mt, case when orders =1 then 'a. 1' when orders <=7 then 'b. 2-7' when orders <=14 then 'c. 7-14' when orders <=25 then 'd. 14-25'
          when orders >25 then 'e. >25' end as order_bucket, count(distinct user_id) users, sum(orders) total_orders
          from (select user_id,mt, count(distinct order_code) orders
          from a
          group by 1,2) m
          group by 1,2 ;;
  }
  ...
  measure: a_1 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_order_bucket_cm.a_1_html._rendered_value}} </p>;;
  }
  ...
  measure: b_2_7 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_order_bucket_cm.b_2_7_html._rendered_value}} </p>;;
  }
  ...
  measure: c_7_14 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_order_bucket_cm.c_7_14_html._rendered_value}} </p>;;
  }
  ...
  measure: d_14_25 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_order_bucket_cm.d_14_25_html._rendered_value}} </p>;;
  }
  ...
  measure: e_25 {
    ...
    value_format: "0.0%"
    html: <p> {{vas_ageing_order_bucket_cm.e_25_html._rendered_value}} </p>;;
  }
